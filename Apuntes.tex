...
\begin{definition}
	...
	\begin{enumerate}[\quad i)]
		...
		\item Existe $ \varphi' (t)$, $\forall t \in I$ (derivada lateral en caso de que $ I $ contenga un extremo).
...
\begin{proof}
	...
	Supongamois ahora cierto para un $r$ arbitrario y probémoslo para $r+1$. Sea $f \in C^{r+1} (A)$ (por lo que $f \in C^r \stackrel{\text{hip.}}{\Rightarrow}$ $\varphi \in C^{r+1}$). Veamos que $\varphi \in C^{r+1}$.
...
\begin{proof}[Solución]
	...
	\begin{figure}[h]
		...
		\draw[line width=1pt,smooth,samples=100,domain=-2.2696161464380893:5.5453285056459745] plot(\x,{0-1/((\x)-5/4)});
		
		\draw (1.371004410996194,0.7087831877032161) node[anchor=north west] {x=5/4};
		\end{axis}
		\end{tikzpicture}
		\caption{Solución del ejemplo.} \label{E1}
	\end{figure}
	...
	también es solución por la misma razón que $h$, mientras que:
...
\begin{proof} \textbf{Caso I.}
	...
	\end{enumerate}
	Por lo tanto, podemos afirmar que $g$ es continua en $J$. Además $\pi$ es derivable en $\mathring{J}$ y $F$ es derivable en $J$, por lo que $g$ es derivable en $\mathring{J}$. Con esto y con que $gF$ es derivable en $\mathring{J}$ tenemos que, por el Teorema de Incrementos Finitos\textsuperscript{1}:
	%\textsuperscript{1} El Teorema de Incrementos Finitos dice que dada cualquier función $f$ continua en el conjunto $A$ y diferenciable en $\mathring{A}$ entonces existe al menos algún punto $\xi$ en $\mathring{A}$ tal que $\norm{f(t)-f(t_0)}\leq \sup\norm{f'(\xi)}|t-t_0|$ QUIERO PONER ESTO A PIE DE PAGINAAAAAAAA
	%----------------------------------------------------------
...
\end{proof}
\begin{theorem}
	Dada la ecuación $x' = f(t, x)$ (*) con $f$ continua y $\varphi :t \in I \subset \mathbb{R} \longrightarrow \mathbb{R}^n$ tal que $(t, \varphi (t)) \in A$ $\forall t \in A$ se tiene:
	\begin{center}
...
\begin{proof}
	...
	$"\Leftarrow"$ \\
	Ahora supongo que se cumplen $(1)$ y $(2)$. Por hipótesis $(t, \varphi (t)) \in A$ $\forall t \in I$. Pero, ¿$\exists \varphi' (t)$ $\forall t \in I$?\\
	Tenemos en primer lugar que $f$ continua y que $\varphi$ es continua, lo que implica que $\int_{t_0}^{t} f(s, \varphi (s))ds$ es derivable y su derivada vale $f(t, \varphi (t))$. Por tanto $\varphi (t)$ es derivable y $(\varphi (t_0))' = 0$. \\
	Por tanto, $\varphi' (t) = f(t, \varphi (t))$.
\end{proof}
\begin{theorem}\label{thm:sucesion}
	Sea $\{\varepsilon_m\}$ sucesión de números reales $\varepsilon_m \geq 0$ tal que $\{\varepsilon_m\}_{m\in \mathbb{N}}$ y sea $I \subset \mathbb{R}$ intervalo compacto. Dado (*) con $A$ abierto, $f$ continua en $A$ y supongamos que $\exists \pi_m : I \longrightarrow \mathbb{R}^n$ solución $\varepsilon$-aproximada de (*). Entonces, si $\exists \varphi : t \in I \longrightarrow \mathbb{R}^n$ tal que $(t, \varphi (t)) \in A$ $\forall t \in I$ y $\{\pi_m\}_{m\in \mathbb{N}}$ converge a $\varphi$ uniformemente en $I$, entonces $\varphi$ solución de (*). 
\end{theorem}

\begin{definition}
	Sea $I \subset \mathbb{R}$ intervalo compacto, $M \subset C(I, \mathbb{R})$ es equicontinuo si:
	\[\forall \varepsilon > 0 \hspace{0,2cm} \exists \delta > 0 \hspace{0,2cm} / \hspace{0,2cm} \forall x, y\in |x-y| < \delta \Rightarrow \norm{h(x) - h(y)} < \varepsilon\]
	$\forall h \in M$.
\end{definition}
\begin{example}
	\begin{enumerate}
		...
		\item Un conjunto de funciones lipschitziana con la misma constante de Lipschitz $K$.
		\begin{center}
...
\begin{definition}
	Sea \begin{align*}
	f: A \subset \mathbb{R} \times \mathbb{R}^n& \longrightarrow \mathbb{R}^n \\
	(t, x)& \longrightarrow f(t,x)
	\end{align*} y $x' = f(t, x)$ (*). Decimos que (*) tienen solución única pasando por $(t_0, x_0) \in A$ si cualquiera que sean $\varphi_1 : I_1 \longrightarrow \mathbb{R}^n$ y $\varphi_2 : I_2 \longrightarrow \mathbb{R}^n$ soluciones pasando por $(t_0, x_0)$ si cumple que $\varphi_1 (t) = \varphi_2 (t)$ $\forall t \in I_1 \cap I_2$.
\end{definition}
\begin{definition}
	Sea \begin{align*}
	f: A \subset \mathbb{R} \times \mathbb{R}^n& \longrightarrow \mathbb{R}^n \\
	(t, x)& \longrightarrow f(t,x)
	\end{align*}. Diremos que $f$ es lipschitziana en $A$ con respecto a la variable $x$ si $\exists k \in \mathbb{R}$, $k \geq 0$ tal que:
	\[\norm{f(t, x) - f(t, y)} \leq k\norm{x-y} \hspace{0,2cm} \forall (t, x), (t, y) \in A\]
	Se denota $f\in L(A, x)$ o bien $f \in L_k (A, x)$.
\end{definition}
\begin{example}
...
\end{note}
\begin{definition}
	Sea $f: A \subset \mathbb{R} \times \mathbb{R}^n \longrightarrow \mathbb{R}^n$. Decimos que $f$ es localmente lipschitziana en $A$ con respecto a la variable $x$ si $\forall (t_0, x_0) \in A$, $\exists\xi_0$ entorno de $(t_0, x_0)$ tal que $f \in L(\xi_0, x)$. Se denota como $f \in L_{loc}(A, x)$.
\end{definition}
\begin{theorem}
...
\begin{proof}
	\begin{enumerate}
		...
		\item De la misma forma que para el punto 1).
	\end{enumerate}
\end{proof}
\begin{definition}
	Sea $(M, d)$ un espacio métrico y sea $T: M \longrightarrow M$, $0 \leq \lambda < 1$. Se dice que $T$ es una $\lambda$-contracción si $\forall x, y \in M$, 
	\[d(T(x), T(y)) \leq k \, d(x, y) \stackrel{k < 1}{\Leftrightarrow} d(T(x), T(y)) < d(x, y)\]
\end{definition}
\begin{theorem}
	(Teorema de la transformada contractiva). Sea $(M, d)$ un espacio métrico completo y $T: M \longrightarrow M$ una $\lambda$-contracción entonces $T$ tiene un único punto fijo, es decir,
	\[\exists! \, x \in M \,/\, T(x) = x\]
\end{theorem}
\begin{theorem}
	(Teorema de Picard - Lipschitz). Sea $x' = f(t, x)$, $f : A \subset \mathbb{R} \times \mathbb{R}^n \longrightarrow \mathbb{R}^n$. Si:
	\begin{enumerate}[i)]
		\item $A$ abierto.
		\item $f \in C(A)$.
...
\end{theorem}
\begin{note}
	Si $f(t, x) = x' = x^{\frac{2}{3}}$ tenemos que es continua y no tiene solución única, entonces $f \notin L_{loc}$. \\
	En $(t, 0)$ $f \notin L_{loc}$ pues si $\xi$ es un entorno de $(t, 0)$, tomando $(t, x), \, (t, y) \in \xi$, con $y=0$:
	\[\frac{\abs{f(t, x) - f(t, y)}}{\abs{x - y}} = \abs{\frac{x^{\frac{2}{3}}}{x}} = \abs{x^{-\frac{1}{3}}}\]
	y como:
	\[\lim\limits_{x \to 0} \frac{1}{x^{\frac{1}{3}}} = +\infty\] 
	vemos lo que afirmamos al principio.
\end{note}

	BEGIN JORGE 25/02
\begin{proof}
	La demostración se basa en dos resultados, el teorema de la transformada contractiva (CITAR) y el teorema de caracterización de la solución (CITAR). En primer lugar definiremos una serie de elementos que se emplearán a lo largo de la demostración:
	\begin{enumerate}
		\item Sea $ B_1 $  entorno abierto de $ (t_0,x_0) $ en el cual $ f $ está acotada. Sea $ H $ cota para $ f $.
	...
	Después de todo esto definimos el conjunto:
	 \[X=\{\varphi : I=[t_0-\delta, t_0+\delta]\longrightarrow \mathbb{R}^n : \varphi \text{ continua, } ||\phi(t)-x_0||\leq H \delta \hspace{0,2cm} \forall t \in I  \} \]
	...
	\[\begin{rcases}
		\text{Se tiene que }C(I, \mathbb{R}^n) \text{ espacio métrico completo}\\
		X = B_{C(I)} [x_0, H\delta] \subset C(I, \mathbb{R}^n) \text{ cerrado } \\
	\end{rcases} \Rightarrow (X, d) \text{ completo.}\]
	Definimos ahora, 
	\[T:\varphi \in X \longrightarrow T\varphi = x_0 + \int_{t_0}^{t} f(s, \varphi(s))ds\]
	Quiero usar el teorema de la aplicación contractiva, por tanto, tengo que demostrar:
	\begin{enumerate}
		\item $T\varphi \in X$.
		\item $T$ en $\lambda$-contracción.
	\end{enumerate}
	Para ver que $T\varphi \in X$ tengo que ver:
	\begin{enumerate}[	i.]
		\item $T\varphi$ continua
		\item $\norm{T\varphi (t) - x_0} \leq H\delta$, $\forall t \in I$
	\end{enumerate}
	En primer lugar, tenemos que:
	\[s \stackrel{g}{\longrightarrow} (s, \varphi (s)) \stackrel{f}{\longrightarrow} f(s, \varphi (s))\]
	por lo que, $T\varphi$ continua por composición de funciones continuas. \\
	En segundo lugar, 
	...
	tomando $\lambda = \delta k$, como $\delta < \frac{1}{k}$ tenemos que $\lambda < 1$.
	Por lo tanto, $T$ es $\lambda$-contracción. Por tanto, por el Teorema de la transformada contractiva:
	\[\exists ! \varphi \in X : \varphi \text{ punto fijo de } T\]
	por lo que $\varphi (t) = T\varphi (t) = x_0 + \int_{t_0}^{t} f(s, \varphi (s))ds$, con $\varphi$ continua.
...
\begin{proof}
	...
	\begin{figure}[h]
		...
		\coordinate[label=above:A] (A) at (5.2,2.8); %etiqueta de A
		\coordinate[label=above:R] (R) at (4.2,2.2); %etiqueta de R
		\draw  plot[smooth,tension=.7] coordinates {(1.25,1.75) (1.5,2.2) (1.7,2) (2.25,2.6) (2.8,0.75) (3.2, 2.6) (3.4, 1.9) (3.7, 2.3)}; %linea poligonal
		\fill (2.5,1.75)  circle[radius=1pt]; %punto en t0,x_0
		\coordinate[label={[scale=0.6] right:$(t_0,x_0)$}] (p0) at (2.5,1.7); %etiqueta de t0 x0
		\end{tikzpicture}
		\caption{Esquema de la demostración.} \label{M1}
	...
	Probémoslo por reducción al absurdo. Supongamos que $ \exists H_1\in I : ||\psi(t_1)-x_0||>H\delta$, por ser $ \psi $ continua $ \exists t_2\in (t_0,t_1) $ en el cual $ ||\psi(t_2)-x_0||H\delta $.
	...
	Veamos que esto tampoco puede pasar. Supongamos que $ \varphi_1: I_1\longrightarrow\mathbb{R}^n, \varphi_2:I_2 \longrightarrow \mathbb{R}^n $ son soluciones de $ x'=f(t,x) $ pasando por $ (t_0,x_=) $ y $ \exists t\in I_1\cap I_2 : \varphi_1(t)\neq \varphi_2(t) $.
	...
	Hacemos en $ (t_1, x_1) $ lo mismo que en $ (t_0,x_0) $ (construyo el rectángulo, etc...) y llego a que en $ (t_1-\delta_1, t_1+\delta _1), \varphi_1(t)=\varphi_2(t) $ y esto NO es posible ya que, por construcción, $ t_1=\inf \{t\in I_1\cap I_2 : t>t_0, \varphi_1(t)\neq \varphi_2(t)\} $.
\end{proof}
...
\begin{note}
	Los teoremas de Cauchy-Peano como Picard-Lipschitz definen existencia y unicidad de soluciones a nivel local, en un entorno arbitrariamente pequeño. Son preguntas naturales: \begin{itemize}
...
\begin{definition}
	Sean $ A $ un abierto de $ \mathbb{R}^{n+1} $ y $ f $ continua:
	\[ f:(t,x)\in A\subset \mathbb{R}\times \mathbb{R}^n \longrightarrow f(t,x)\in \mathbb{R}^n \]
	con $  x'=f(t,x) $ su ecuación diferencial asociada.
	
	Sean además $ \varphi_1:I_1\longrightarrow \mathbb{R} $ y  $ \varphi_2:I_2\longrightarrow \mathbb{R} $ soluciones de dicha ecuación diferencial. Diremos que $ \varphi_2 $ es una prolongación de $ \varphi_1 $ si:
	\begin{enumerate}
		\item $ I_1\subseteq I_2 $
		\item $ \varphi_1(t) =\varphi_2(t) \forall t \in I_1$
	\end{enumerate}

	Además, si $ I_1 \varsubsetneq I_2 $ diremos que $ \varphi_2 $ es una prolongación propia de $ \varphi_1 $.
\end{definition}
\begin{note}
	Una solución es prolongación de si misma, pero no es prolongación propia.
\end{note}
\begin{definition}
	Si $ \varphi $ es solución de una ecuación diferencial, diremos que $ \varphi $ es maximal si no admite prolongación propia. En este caso, el intervalo de definición es llamado intervalo maximal.
\end{definition}
...
\begin{proof}
	...
	Trabajamos solo con soluciones que sean prolongación de $ \varphi_0 $. Es un conjunto no vacío, ya que $ (I_0,\varphi_0) $ es prolongación de si mismo.Utilizamos la relación de orden parcial definida anteriormente y comprobaremos que se cumplen las hipótesis del Lema de Zorn.
...
\begin{definition}
	...
	Sean $ \varphi_1:(a,b)\longrightarrow \mathbb{R}^n $ y $ \varphi_2:(c,d)\longrightarrow \mathbb{R}^n $ soluciones de dicha ecuación diferencial. Si $ \varphi_2 $ es una prolongación de $ \varphi_1 $ y $ b\in(c,d) $, diremos que $ \varphi_2 $ es prolongación de $ \varphi_1 $ a través de $ b $.
\end{definition}
...
\begin{definition}[General]
	...
	Sean $ \varphi_1:I_1\longrightarrow \mathbb{R}^n $ y $ \varphi_2:I_2\longrightarrow \mathbb{R}^n $ soluciones de dicha ecuación diferencial. Si $ \varphi_2 $ es una prolongación de $ \varphi_1 $ y existe $ q\in \text{Fr}(I_1) $ tal que $ q\in \mathring{I_2}$, diremos que $ \varphi_2 $ es prolongación de $ \varphi_1 $ a través de $ q $.
\end{definition}
...
\begin{proof}
	\begin{enumerate} [i)]
		...
		Como $\lim\limits_{t \to b^-} \varphi (t) = p$ y, por ser continua, es secuencialmente continua, lo que implica que existe una sucesión $\{t_m\} \longrightarrow b^-$ tal que $\{\varphi(t_n) \longrightarrow p\}$. Elegimos entonces un $n_0 \in \mathbb{N}$ tal que $\abs{t_{n_0} - b} < \delta$ y $(t_{n_0}, \varphi (t_{n_0})) \in \overline{B}$. Para la condición inicial $(t_{n_0}, \varphi (t_{n_0}))$ existe una solución única pasando por el punto definido en $[t_{n_0} - \delta, t_{n_0} + \delta]$ que denoto $\overline{\varPhi}$. Entonces la función:
...
\begin{observation}
	...
	Llega trabajar con una sucesión. Además, no es necesario pedir que $f \in L_{loc}(A, x)$, sin ella no podemos concluir la unicidad y la demostración sería mucho más complicada.
\end{observation}
\begin{corollary}
	Si $A$ está acotado, yo puedo prolongar hasta la frontera.
\end{corollary}
\begin{theorem}
	(Teorema de la banda) \\
	Dada $x' = f(t, x)$, $f: (t, x) \in \mathbb{R} \times \mathbb{R}^n \longrightarrow f(t, x) \in \mathbb{R}^n$ siendo $A = (a, b) \times \mathbb{R}^n$. Si se cumpla una de las siguientes condiciones:
	\begin{enumerate}
		\item  $f$ continua en $A$, $f \in L_k (A, x)$ ó
		\item  $f$ continua y acotada en $A$ ($f \in L{loc} (A, x)$).
	\end{enumerate}
	
	DIBUJO
	
	entonces, fijado $(t_0, x) \in (a, b) \times \mathbb{R}^n$, la solución maximal que pasa por él, está definida en $(a, b)$.
\end{theorem}
\begin{observation}
	\begin{enumerate}
		\item No es necesario pedir $f \in L_{loc} (A, x)$.
		\item $a$, $b$ no tienen porque ser finitos, es decir, puedo tener intervalos de la forma $(- \infty, b)$.
	\end{enumerate}
\end{observation}
\begin{theorem}
	Dado $x' = f(t, x)$, $f: [a, b] \times \mathbb{R}^n \longrightarrow \mathbb{R}^n$. Si se cumple una de las siguientes condiciones:
	\begin{enumerate}
		\item $f$ continua en $A = [a, b] \times \mathbb{R}^n$, $f \in L_k (A, x)$ ó
		\item $f$ continua en $A$, $f \in L_{loc} (A, x)$ entonces, fijado $(t_0, x) \in A$ la solución pasando por ese punto y la solución maximal está definida en $[a, b]$. 
	\end{enumerate}
\end{theorem}
\begin{proof}
	...
	\end{enumerate}
	Por tanto, fijado $(t_0, x_0) \in A \subset (c, d) \times \mathbb{R}^n$ existe la solución maximal pasando por $(t_0, x_0)$ y definida en $(c, d)$. Por tanto,
	\[\varphi = \psi|_{[a, b]} \text{es la solución maximal}\]
	de $x' = f(t, x)$ pasando por $(t_0, x_0)$.
...
\subsection{Dependencia de la solución respecto de las soluciones iniciales.}
...
\begin{exercise}
	Razona veracidad o falsedad:
		\begin{enumerate}[\quad a)]
			\item Si $ \varphi_1, \varphi_2 $ son soluciones de (1), definidas en $ [a, t_0], [t_0,b] $, y además sea $ \varphi_1(t_0)=\varphi_2(t_0) $, entonces $ \varphi $ definida en $ [a,b] $ como:
			\[ \varphi(t)=\begin{cases}
			\varphi_1(t), \quad t\in [a,t_0]\\
			\varphi_2(t), \quad t\in [t_0,b]
			\end{cases} \]
			es solución de (1).
			\item Sea (1), entonces toda solución maximal está definida en un abierto.
		\end{enumerate}
\end{exercise}
\begin{proof}[Solución]
	\begin{enumerate}[\quad a)]
			\item Verdadero. Veamos si $ \varphi $ es solución.
			\begin{enumerate}[\quad i)]
				...
				Como $ \varphi_1(t_0)=\varphi_2(t_0) \Rightarrow \lim\limits_{t \to t_0^-} \varphi'(t)=\lim\limits_{t \to t_0^+} \varphi'(t)$, y $\varphi $ continua en $ t_0 $, entonces $ \exists \varphi'(t_0) $
				\item Inmediata visto lo anterior.
			\end{enumerate} 
		\item Probemos que es falso por reducción al absurdo. Supongamos que está definida en un intervalo de la forma $ (a,b] $.
		Como $ A $ es abierto, $ (b,\varphi(b))\in A \Rightarrow (b, \varphi(b)) \in \mathring{A} $.
		
		Estamos en la hipótesis de Cauchy-Peano. Por lo tanto, pasando por $ (b, \varphi(b)) $ existe solución. Podemos construir una prolongación que sea solución (por el apartado a) y entonces no es maximal.
		\end{enumerate}
\end{proof}
...
\end{theorem}
\begin{proof}
	Se hará en un laboratorio COMPLETAR.
\end{proof}
\begin{definition}
