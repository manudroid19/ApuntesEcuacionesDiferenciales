...
\begin{proof}\ \\
	...
	\begin{enumerate}
		...
		\item $\norm{f(t, x) - f(t, y)} \leq k \norm{x - y}$
	\end{enumerate}
	ya que $A(t)$ es continua en un compacto, por lo que $A(t)$ es acotada. Por tanto, la solución maximal está definida en $[c, d]$. Compruebo ahora que dado un punto $(t_0, x_0) \in I \times \mathbb{R}^n$ y vemos que la solución maximal está definida en $I$. \\
	Sea $t \in I$ fijado arbitrario y escojo un compacto $[c, d]/subset I$ tal que $t, t_0 \in [c, d]$ \\
	\begin{center}
		DIBUJO
	\end{center}
	Por el resultado probado tendría que la solución está definida en $[c, d]$ y, por tanto, en $t$. Si esto lo repito para todo $t \in I$, tenemos que la solución maximal está definida en $t \in I$.
\end{proof}
\begin{note}
...
\begin{proof}
	...
	\begin{align*}
		...
		&\varphi \longrightarrow F(\varphi) = \varphi (t_0)
	\end{align*}
...
\begin{proof}\ \\
	...
	\end{enumerate}
	"$ \Rightarrow$": Sea $\psi (t)$ matriz fundamental y vemos que:
	\[\Delta = \phi^{-1} (t) \psi(t)\]
	verifica:
	\begin{enumerate}
		\item  $\psi = \phi \Delta$. Esto se debe a que:
		\[\phi(t) \varOmega (t) = \phi(t) \phi^{-1}(t) \psi(t) = \psi(t) \]
		\item  $\varOmega$ es constante. Para ver esto veamos que $\varOmega' = 0$.
		\begin{align*}
			\Delta' &= (\phi^{-1} \psi)' = (\phi^{-1})'\psi + \phi^{-1}\psi' \\ 
	...
	\end{enumerate}
\end{proof}
Sea ahora el sistema completo (S.C.) $x' = A(t)x + b(t)$, tal que:
\begin{align*}
	A &: t \in I \subset \mathbb{R} \longrightarrow A(t) \in M_{n \times n} (\mathbb{R}) \\
	b &: t \in I \subset \mathbb{R} \longrightarrow b(t) \in \mathbb{R}^n
\end{align*}
siendo $A$, $b$ continuas.\\
Sea $S_H$ el espacio vectorial de soluciones de $x' = Ax$. Sea además $S_C$ el conjunto de soluciones del sistema completo $x' = Ax + b$.
\begin{theorem}
	Sea $\varphi \in S_H$. Entonces tenemos que:
	\begin{center}
		$S_C = \{ \varphi^C + \varphi \ : \ \varphi^C \text{ solución particular del S.C. } x' = A(t)x + b \} = \varphi^C + S_H$
	\end{center}
\end{theorem}
\begin{proof}\ \\ 
	...
	\begin{enumerate}
		...
		\item $\varphi^C$ solución particular completo.
	\end{enumerate}
	Veremos que estas dos condiciones implican que $\varphi + \varphi^C$ solución del sistema completo. Esto se debe a que:
	\begin{center}
	...
	\end{center}
\end{proof}
\begin{theorem}[Fórmula de Lagrange o método de variación de constantes]
	La solución de S.C. $x' = A(t)x + b(t)$ que pasa por $(t_0, x_0)$ viene dada por:
	\[x(t) = \phi(t) \phi^{-1}(t_0) x_0 + \int_{t}^{t_0}\phi(t) \phi^{-1}(s) b(s) \ ds\]
	donde $\phi(t)$ es matriz fundamental para el S.H.
\end{theorem}
\begin{proof}
	La solución general de S.H. vendría dada por $x(t) = \phi (t) c$, $c \in \mathbb{R}^n$. Buscamos una solución de S.C. de la forma $x(t) = \phi (t) c(t)$, $c(t) \in \mathbb{R}^n$.\\
	Para que sea solución del S.C. ha de verificar:
	\begin{align*}
		&\phi'(t)c(t) + \phi(t)c'(t) = A(t)\phi(t)c(t) + b(t) \\
		\Leftrightarrow \ &A(t)\phi(t)c(t) + \phi(t)c'(t)= A(t)\phi(t)c(t) + b(t) \\
...
\end{proof}
\section{Tema 6.}
\subsection{Sistemas de ecuaciones lineales con coeficientes constantes.}
\begin{definition}
	Llamamos \emph{sistema lineal de coeficientes constantes} a una ecuación diferencial de la forma:
	\[ x'=A(t)x+b(t) \]
	con $ x'\in \mathbb{R}^n, x\in\mathbb{R}^n $, $ A\in M_{n\times n} $ constante y $ b:t\in I\subset\mathbb{R}\longrightarrow b(t)\in\mathbb{R}^n $ continua.
\end{definition}
\begin{definition}
...
\begin{proof} \ \\
	...
	\begin{enumerate}[\quad i)]
		...
		\begin{enumerate}[\quad a)]
			\item $ e^{tA} $ converge para $ t $ fijo.
		...
		Por último, para c), como para dado $ t\in \mathbb{R} $, existe $ J $ compacto tal que $ t\in J $. Por tanto, existe $ \frac{\partial}{\partial t }e^{tA} \forall t \in \mathbb{R}$, con $ \frac{\partial}{\partial t }e^{tA}=Ae^{tA}, t\in\mathbb{R}$.
		
		Así, $ e^{tA} $ es matriz solución.
		
		\item $ e^{tA} $ es fundamental. Tomemos $t = 0$:
		\[det(e^{tA}) = det(e^0) = det(I) = 1 \neq 0\]
		Por tanto, por el teorema de caracterización de matrices fundamentales, $e^{tA}$ es fundamental.
	\end{enumerate}
\end{proof}
\begin{theorem}
...
\end{proof}
\begin{theorem}
	Sea $x' = Ax + b(t)$ con $A \in M_{n \times n}$ constante y:
	\[b:I \subset \mathbb{R} \longrightarrow \mathbb{R}^n\]
	continua con $I$ abierto. La solución pasando por $(t_0, x_0)$ vendrá dada por:
	\[x(t) = e^{(t - t_0)A}x_0 + \int_{t}^{t_0}e^{(t - s)A}b(s) \ ds.\]
\end{theorem}
\begin{proof}
	Por la fórmula de Lagrange y teniendo en cuenta que $\phi (t) = e^{tA}$, tenemos que:
	\begin{align*}
	...
	\end{align*}
	Tenemos que se cumple la igualdad $1)$ ya que $(e^A)^{-1} = e^{-A}$ y la igualdad $2)$ ya que $t_0A$ y $tA$ conmutan y porque $tA$ y $sA$ también conmutan.
\end{proof}
\begin{theorem}
