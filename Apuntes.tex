...
\begin{definition}
	La ecuación diferencial dada por (\ref{eqEx}) es una \emph{EDO exacta} si $\exists F(t, x)$ tal que $\frac{dF}{dt} = M$ y $\frac{dF}{dx} = N$. A $F$ se le denomina \emph{función potencial}.
\end{definition}
\begin{proposition}
...
\begin{proof}
	...
	"$\Leftarrow$" \\
	Supongamos que $\frac{dM}{dx} = \frac{dN}{dt}$  y construyo $F$ función potencial, es decir, $F$ tal que $\frac{dF}{dt} = M$, $\frac{dF}{dx} = N$. Los candidatos que tenemos son:
	\[\int M(t, x)dt + g(x)\]
	y
	\[\int N(t, x)dx + h(t).\]
	Sea entonces, por ejemplo, $F(t, x) = \int M(t, x)dt + g(x)$, ¿existe entonces $g$ tal que $\frac{dF}{dx} = N$? Vemos que:
	...
	Por tanto basta con ver que esta expresión no depende de $t$:
	\begin{align*}
		& \frac{d}{dt}(N(t, x) - \frac{d}{dx}\int M(t, x)dt) =  \frac{d}{dt}N(t, x) -  \frac{d}{dt}\frac{d}{dx}\int M(t, x)dt = \\
		& \frac{d}{dt}N(t, x) -  \frac{d}{dx}\frac{d}{dt}\int M(t, x)dt = 
		\frac{dN}{dt} - \frac{dM}{dt} = 0
	\end{align*}
	siendo esta última igualdad producto de la hipótesis inicial que nos afirmaba que $\frac{dM}{dx} = \frac{dN}{dt}$. Por tanto, 
	\[g(x) = \int[N(t, x) - \frac{d}{dx}\int M(t, x)dt]dx\]
	teniendo así garantizado que $F$ es exacta.
\end{proof}
\paragraph{Método de resolución} \ 

\subsubsection{EDOs resolubles por factor integrante.}
Despúes de haber visto la resolución de las ecuaciones exactas nos surge la duda, como se resuelven las ecuaciones del estilo $xdt - tdx = 0$ ya que no es exacta, pero si la multiplico por $\frac{1}{t^2}$, con $t \neq 0$:
\[(\frac{1}{t^2}x)dt + \frac{1}{t^2}(-t)dx = 0 \Leftrightarrow \frac{x}{t^2}dt + (-\frac{1}{t})dx = 0\]
donde
\[\frac{dM}{dx} = \frac{1}{t^2} = \frac{dN}{dt}.\]
\begin{definition}
	Diremos que $\mu : (t, x) \in I_1 \times I_2 \longrightarrow \mathbb{R}$ es un factor integrante para $M(t, x)dt + N(t, x)dx = 0$ si:
...
\end{definition}
\begin{enumerate}
	\item Supongamos que $\mu$ depende solo de t, $\mu(t)$. Entonces, la ecuación (\ref{eqInte}) se reescribe:
	\[\mu(t, x)M_x(t, x) = \mu_t(t, x)N(t, x) + \mu(t, x)N_t(t, x)\]
	por lo que:
	\[\mu(M_x - N_t) = \mu_t N \Rightarrow \frac{\mu_t}{\mu} = \frac{M_x - N_t}{N}.\]
	Si depende sólo de $t$:
	\[\int \mu(M_x - N_t)dt = \int \frac{M_x - N_t}{N}dt.\]
