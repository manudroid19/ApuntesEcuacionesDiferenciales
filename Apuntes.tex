...
\begin{proof}
	...
	$"\Leftarrow"$ \\
	Ahora supongo que se cumplen $(1)$ y $(2)$. Por hipótesis $(t, \varphi (t))$ $\forall t \in I$. Pero, ¿$\exists \varphi' (t)$ $\forall t \in I$?\\
	Tenemos en primer lugar que $f$ continua y que $\varphi$ es continua, lo que implica que $\int_{t_0}^{t} f(s, \varphi (t))ds$ es derivable y su derivada vale $f(t, \varphi (t))$. Por tanto $\varphi (t)$ es derivable y $\varphi' (t_0) = 0$. \\
	Por tanto, $\varphi' (t) = f(t, \varphi (t))$.
\end{proof}
\begin{theorem}
	Sea $\{\varepsilon_m\}$ sucesión de números reales $\varepsilon_m \geq 0$ tal que $\{\varepsilon\}_{m\in \mathbb{N}}$ y sea $I \subset \mathbb{R}$ intervalo compacto. Dado (*) con $A$ abierto, $f$ continua en $A$ y supongamos que $\exists \pi_m : t \in I \longrightarrow \mathbb{R}^n$ solución $\varepsilon$-aproximada de (*). Entonces, si $\exists \varphi : t \in I \longrightarrow \mathbb{R}^n$ tal que $(t, \varphi (t)) \in A$ $\forall t \in I$ y $\{\pi_m\}$ converge a $\varphi$ uniformemente en $I$ e implica que $\varepsilon$ solución de (*). 
\end{theorem}

\begin{definition}
	Sea $I \subset \mathbb{R}$ intervalo compacto, $M \subset C(I, \mathbb{R})$ es equicontinuo si:
	\[\forall \epsilon > 0 \hspace{0,2cm} \exists \delta > 0 \hspace{0,2cm} / \hspace{0,2cm} \forall x, y\in |x-y| < \delta \Rightarrow \norm{h(x) - h(y)} < \varepsilon\]
	$\forall h \in M$.
\end{definition}
\begin{example}
	\begin{enumerate}
